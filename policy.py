...
class BoltzmannPolicy():
    def __init__(self, session, num_iterations, temp_init=1, temp_final=0.5):
        self.session = session 
        self.temp_decrement = (temp_init-temp_final)/num_iterations
        self.temp_func = lambda t : -self.temp_decrement*t + temp_init
        self.cur_val = temp_init

    def select_action(self, current_q_func, input_batch, current_observation, time):
        q_vals = self.session.run(current_q_func, {current_observation: input_batch[None, :]})
        self.cur_val = self.temp_func(time)
